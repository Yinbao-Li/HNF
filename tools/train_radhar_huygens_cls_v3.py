#!/usr/bin/env python
"""RadHAR v3: max-perf wave backbone + full rich features.

Changes over v2:
  - ConvStem: 312ch → 64ch before wave blocks (local temporal context)
  - EnergyPoolHead: mean+max+peak pooling (richer readout)
  - embed_dim 128, 4 wave layers, medium_hidden 64
  - Label smoothing 0.05
  - MixUp alpha=0.3
  - Cosine LR with warm restarts (SGDR-style: 1 restart mid-training)
  - Larger augmentation (scale ±20%, shift ±8, noise ±0.08)

  python tools/train_radhar_huygens_cls_v3.py --device cuda
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=Path, default=Path("outputs/mmwave/radhar_huygens_cls_v3"))
    ...
    p.add_argument(
        "--prev-summary",
        type=Path,
        default=Path("outputs/mmwave/radhar_huygens_cls_v2/SUMMARY.json"),
    )
    return p.parse_args()
...
def main() -> None:
    ...
    print(
        f"[v3] train={len(train_ds)} test={len(test_ds)} "
    ...
    model = RadHARHuygensClsModel(
        ...
        dropout=0.3,
        medium_hidden=64,
        # keep original v3 recipe even after v4 default flip
    ...
    print(f"[v3] params={n_params:,}", flush=True)
    ...
    summary = {
        "dataset": "RadHAR",
        "version": "v3_convstem_energypool",
    ...
    lines = [
        "# RadHAR Huygens/wave v3 (ConvStem + EnergyPool)",
        "",
