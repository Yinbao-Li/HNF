#!/usr/bin/env python
"""RadHAR v2: rich range–Doppler features + Huygens/wave cls (fixed backbone).

Improvements over v1:
  - Multi-cue input (intensity, velocity, bearing, range–Doppler)
  - Disk cache (parse once)
  - Train augment, class weights (boost jump/jack)
  - Cosine LR, longer training

  python tools/train_radhar_huygens_cls_v2.py --device cuda
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=Path, default=Path("outputs/mmwave/radhar_huygens_cls_v2"))
    ...
    p.add_argument("--no-range-doppler", action="store_true")
    p.add_argument("--no-class-weights", action="store_true")
    p.add_argument("--no-augment", action="store_true")
    p.add_argument("--max-files", type=int, default=None)
    p.add_argument("--num-workers", type=int, default=2)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--baseline-summary", type=Path, default=Path("outputs/mmwave/radhar_huygens_cls_v1/SUMMARY.json"))
    return p.parse_args()
...
def main() -> None:
    ...
    ds_kw = dict(
        ...
        feature_mode="rich",
        include_range_doppler=not args.no_range_doppler,
        n_doppler_bins=args.n_doppler_bins,
        cache_dir=args.cache_dir,
        rebuild_cache=args.rebuild_cache,
    )
    train_ds = RadHARClsDataset(
        args.data_dir, "train", augment=not args.no_augment, seed=args.seed, **ds_kw
    )
    ...
    if not args.no_class_weights:
        ...
        # extra boost for jump/jack confusion pair
        for act, boost in (("jump", 1.35), ("jack", 1.15)):
            class_w[ACTIVITY_TO_IDX[act]] *= boost
        class_w = class_w / class_w.mean()

    print(
        f"[v2] train={len(train_ds)} test={len(test_ds)} C={train_ds.n_channels} "
        f"T={args.t_steps} rd={not args.no_range_doppler} aug={not args.no_augment}",
        flush=True,
    )
    ...
    if args.baseline_summary.exists():
        try:
            v1_acc = json.loads(args.baseline_summary.read_text()).get("best", {}).get("accuracy")
        except Exception:
            pass

    summary = {
        "dataset": "RadHAR",
        "version": "v2_rich_doppler",
        "method": "fixed Huygens/wave frontend",
        "n_train": len(train_ds),
        "n_test": len(test_ds),
        "n_channels": train_ds.n_channels,
        "features": "intensity+vel+bearing+count+range_doppler",
        "best": best,
        "v1_baseline_accuracy": v1_acc,
        "delta_vs_v1": (best["accuracy"] - v1_acc) if v1_acc is not None else None,
        "history": history,
    }
    (args.output_dir / "SUMMARY.json").write_text(json.dumps(summary, indent=2, default=str))

    lines = [
        "# RadHAR Huygens/wave v2 (rich features)",
        "",
        f"Channels: {train_ds.n_channels} | Best test acc: **{best['accuracy']:.4f}** (epoch {best.get('epoch')})",
    ]
    if v1_acc is not None:
        lines.append(f"v1 baseline: {v1_acc:.4f} | delta: **{best['accuracy'] - v1_acc:+.4f}**")
