#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fine-tune L1200 board with P residual offset (+ peak-rerank + late decode).

Post-pick: coarse peak → waveform crop → bounded Δt. Loss only when coarse
is already near GT (gate_sec). FNO / spectral path stays out.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output-dir",
        default="outputs/run_obs_native/obs_4c_p_residual_8ep",
    )
    p.add_argument(
        "--resume",
...
def main() -> None:
    ...
    cmd = [
        ...
        "16",
        "--p-residual-offset",
        "--p-residual-crop-half-bins",
        "30",
        "--p-residual-hidden",
        "32",
        "--p-residual-max-delta-bins",
        "8",
        "--p-residual-loss-weight",
        "1.0",
        "--p-residual-gate-sec",
        "1.0",
        "--p-decode-mode",
    ...
    print("[obs-p-residual] " + " ".join(cmd), flush=True)
    if args.dry_run:
        return
