#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Step 7b / Domain III Stage-1: constitutive ID + θ recovery train+eval."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fluid Stage-1 constitutive launcher")
...
def main() -> None:
    ...
    train_cmd = [
        sys.executable,
        "tools/train_fluid_constitutive.py",
        "--output-dir",
        str(out),
        "--epochs",
        str(args.epochs),
    ...
    eval_cmd = [
        sys.executable,
        "tools/eval_fluid_constitutive.py",
        "--checkpoint",
        str(out / "best.pt"),
        "--output",
    ...
    banner = "[fluid-stage1] " + " ".join(train_cmd)
    print(banner, flush=True)
    if args.dry_run:
        print("[fluid-stage1] eval:", " ".join(eval_cmd))
        return
