#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""run27: from-scratch kitchen-sink physics train (5 epochs).
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="run27 multi-scale Fresnel phys from-scratch")
    p.add_argument("--output-dir", default="outputs/run27/27_ms_fresnel_phys")
...
def main() -> None:
    ...
    banner = "[run27] " + " ".join(cmd)
    print(banner, flush=True)
    print(f"[run27] logging to {log_path}", flush=True)
    if args.dry_run:
        return
