...
# -*- coding: utf-8 -*-
"""
inv05-real: STEAD test waveforms + run20 picks -> Zhizi inversion bridge.

No ground-truth vp/vs on real STEAD — reports:
  - P/S pick MAE vs catalog
  - Zhizi one-shot vp/vs vs L-BFGS from same picks (travel-time misfit)
  - Bridge forward-pass sanity on real (non-Ricker) traces
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--obs-fallback", action="store_true",
...
def main() -> None:
    ...
    for batch in loader:
        ...
        zhizi_tt = time_misfit(zhizi_earth, args.source_depth, distances, obs_tp, obs_ts)

        gn_res = invert_lbfgs_torch(
            base.depths, vp_init, vs_init, q_init,
            args.source_depth, distances, obs_tp, obs_ts,
            max_iter=80,
        )
        if not torch.isfinite(gn_res.earth.vp).all():
            gn_res = invert_hnf_adam(
                base.depths, vp_init, vs_init, q_init,
                args.source_depth, distances,
                {"tp": obs_tp, "ts": obs_ts},
                steps=400,
            )
        gn_tt = gn_res.time_misfit

        row.update({
            "status": "ok",
            "zhizi_vp": zhizi_earth.vp.cpu().tolist(),
            "zhizi_vs": zhizi_earth.vs.cpu().tolist(),
            "gn_vp": gn_res.earth.vp.cpu().tolist(),
            "gn_vs": gn_res.earth.vs.cpu().tolist(),
        ...
        print(
            ...
            f"zhizi_tt={zhizi_tt:.4f} gn_tt={gn_tt:.4f}",
            flush=True,
        )
    ...
    summary = {
        ...
        "zhizi_tt_misfit_mean": finite_mean("zhizi_tt_misfit"),
        "gn_tt_misfit_mean": finite_mean("gn_tt_misfit"),
        "events": rows,
    }
